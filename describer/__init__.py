...
import logging, locale, operator
import gobject
...
class Describer:
    ...
    supportedWidgets = []
    """
    gtk.Label, gtk.ToggleToolButton, gtk.ToolButton, gtk.SeparatorToolItem,
    ...
    def __call__(self, widget):
        if self.isEnabled():
            ...
            if isinstance(widget, QWidget): # GTK.Window, Qt all widgets
    ...
    def getWindowTitle(cls, widgetType, window):
        ...
        """
        if window.isWindow():
          return widgetType + " '" + str(window.windowTitle()) + "'"
        else:
          return "Popup Window"
...
class IdleScheduler:
    ...
    def monitorBasics(self, widget):
        ...
        if widget.isWindow(): # Qt isWindow() returns True for independent windows
            ...
            '''
            if widget.isModal():  # lkk ??? Is this correct for Qt?  Is modality the same thing as WINDOW_POPUP
              return
        else:
            # Don't handle windows this way, showing and hiding them is a bit different
            self._monitorBasics(widget)
        """
        self._monitorBasics(widget) # All windows same way??
    ...
    def _monitorBasics(self, widget):
        ...
        """
        # sensitivity (enabled) changes are QChangedEvent is subclass of QEvent
        self.monitorWidgetEvent(widget, 
                                eventType = QEvent.EnabledChange, 
                                interceptMethod=self.storeSensitivityChange)
